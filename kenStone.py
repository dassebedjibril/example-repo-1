# https://github.com/hkaneko1985/kennardstonealgorithm/blob/master/kennardstonealgorithm.py
...
def kennardstonealgorithm(x_variables, k):
    x_variables = x_variables[x_variables.columns[::10]]
    x_variables = np.array(x_variables, dtype=np.float32)
    original_x = x_variables
    distance_to_average = ((x_variables - np.tile(x_variables.mean(axis=0, dtype=np.float32), (x_variables.shape[0], 1))) ** 2).sum(axis=1)
